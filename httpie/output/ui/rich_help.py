...
def to_help_message(spec: ParserSpec) -> Iterable[RenderableType]:
    yield from render_intro(spec)
    group_rows = build_group_rows(spec.groups)
    yield from render_options(group_rows)
    yield from render_footer(spec)

def render_intro(spec: ParserSpec) -> Iterable[RenderableType]:
...
def build_group_rows(groups) -> dict:
    group_rows = {}
    for group in groups:
        options_rows = []
        for argument in group.arguments:
            if argument.is_hidden:
                continue
            options_rows.extend(process_argument(argument))
        group_rows[group.name] = options_rows
    return group_rows
...
def render_options(group_rows: dict) -> Iterable[RenderableType]:
    options_table = Table(highlight=False, box=None, show_header=False)
    for group_name, options_rows in group_rows.items():
        options_table.add_row(Text(), Text(), Text())
...
def render_footer(spec: ParserSpec) -> Iterable[RenderableType]:
